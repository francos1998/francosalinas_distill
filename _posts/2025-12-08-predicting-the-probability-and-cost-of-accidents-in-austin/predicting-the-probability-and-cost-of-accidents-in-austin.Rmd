...
$$
\widehat{y} = E(\text{Crash Cost} \mid \text{Pre-Crash Conditions})
$$
...
The model estimates:$\widehat{p} = P(\text{crash} = 1 \mid \text{pre-crash environmental features})$
This enables real-time crash risk mapping, hotspot identification, and safety-related applications.

### Modeling Objective:

The binary classification objective: $\widehat{p} = P(\text{crash} = 1 \mid \text{X})$; where X includes temporal, spatial, environmental, and historical crash features.
...
$$
L = -\left[ y \log(\widehat{p}) + (1 - y)\log(1 - \widehat{p}) \right]
$$

Using the gradient $g = \frac{dL}{df}$ and the Hessian $h = \frac{d^{2}L}{df^{2}}$
...
![Cash Probability Hotspot Map](images/hotspotint.png){width=60%}
...
$$\widehat{c} = E\big[\text{count} \mid \text{crash} > 0\ \big]$$
